#ifndef SIM_VECTORF_H_
#define SIM_VECTORF_H_
#include <stdint.h>
#include <stdio.h>
#include "platform.h"
#include "simath.h"
...
static inline float fvec_min(fvec_t const *input)
{
    ...
    tmp = _mm_min_ps(input->data.sse_register, _mm_shuffle_ps(input->data.sse_register,
...
static inline void fvec_print(fvec_t const *input)
{
    SIM_ASSERT(input && input->size > 0 && input->size < 5);
    printf("[ ");
